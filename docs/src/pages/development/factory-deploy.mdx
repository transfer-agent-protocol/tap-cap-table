...
Factory owner is **not** the same as the server `PRIVATE_KEY` used for API-signed txs. See [Three wallets / keys](/development/setup#three-wallets--keys) on Setup. Do not leave the factory-owner key in the always-on API env.
...
Root `.env` needs funded **dev** `PRIVATE_KEY` (the deployer for this run — becomes factory owner), `RPC_URL`, and `CHAIN_ID` (Plume: `98866` / `https://rpc.plume.org`). Prefer a dedicated env file for the deploy command; do not keep the owner key in Docker after deploy.
