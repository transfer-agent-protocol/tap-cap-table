...
In `/app`, open the company workspace at `/app/companies/<issuerId>?view=stakeholders`. The connected ADMIN wallet calls `createStakeholder`, then the app saves OCF metadata with `POST /stakeholder/register-onchain`. The server-signed `/stakeholder/create` tutorial below is the API/automation alternative.
...
| `data.issuer_assigned_id` | Optional. Pass `""` when you have no internal stakeholder ID yet — the field still appears on the record so you can update it later. |
