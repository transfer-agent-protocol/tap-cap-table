...
`docker compose stop server` frees port 8293 and leaves Mongo running. `pnpm dev` starts the API with hot reload and the poller.
...
| Other app wants Mongo 27017 | TAP publishes **27027**. Host `DATABASE_URL` must use 27027. `docker compose stop` the container still bound to 27017. |
