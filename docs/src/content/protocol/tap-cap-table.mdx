...
Contracts store issuer, stock class, stakeholder, position, and transaction state. The product UI writes from the issuer ADMIN wallet; the optional server-signed API can also call the same functions. The poller syncs confirmed events to MongoDB.
