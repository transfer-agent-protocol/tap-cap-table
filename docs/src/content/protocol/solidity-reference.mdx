...
<Callout type="info">
The **API** column is the server-signed `/create` path. The product UI under `/app` writes the same functions from the connected ADMIN wallet, then registers class, stakeholder, and issuance metadata with `/register-onchain`. Transfers have no registration route.
</Callout>
...
| Kind | Names |
| --- | --- |
| Events | `IssuerCreated`, `StakeholderCreated`, `StockClassCreated`, `StockLegendTemplateCreated`, `WalletAdded`, `WalletRemoved`, `SharesAuthorizedMinted`, `ActivePositionMinted`. Lifecycle history is `TxHelper` `TxCreated` (what the poller writes). |
