...
| `security_id` | Canonical id from poller-backed [history](/development/historical-transactions) or active holdings. Not the validation UUID returned by `POST /transactions/issuance/stock`. Transfer itself does not take `security_id`; cancel/retract/reissue/repurchase do. |
