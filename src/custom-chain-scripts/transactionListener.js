...
async function startOnchainListeners(chain, prisma) {
    ...
    let _client;
    if (chain === "local") {
        const { contract, client } = await localSetup();
        _contract = contract;
        _client = client;
    }

    if (chain === "optimism-goerli") {
        const { contract, client } = await optimismGoerliSetup();
        _contract = contract;
        _client = client;
    }
    ...
    _contract.watchEvent.StockIssuanceCreated({
        onLogs: async (logs) => {
            const {
                args: { stock },
                eventName,
            ...
            const block = await _client.getBlock(eventName.blockNumber);
