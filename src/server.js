...
const contractMiddleware = async (req, res, next) => {
    ...
    if (!contractCache[req.body.issuerId]) {
        const { contract, provider, issuanceLib, transferLib, cancellationLib } = await getContractInstance(CHAIN, issuer.deployed_to);
        contractCache[req.body.issuerId] = { contract, provider, issuanceLib, transferLib, cancellationLib };

        // Initialize listener for this contract
        startOnchainListeners(contract, provider, req.body.issuerId, issuanceLib, transferLib, cancellationLib);
    }
