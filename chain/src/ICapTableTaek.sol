...
interface ICapTableTaek {
    // @dev Transactions will be created on-chain then reflected off-chain.
    function transactions(uint index) external view returns (bytes32);
    ...
    function hashToTxEncodedData(bytes32 hash) external view returns (bytes memory);
