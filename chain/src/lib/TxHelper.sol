...
library TxHelper {
    event TxCreated(uint256 index, TxType txType, bytes32 txHash);
    ...
    function createTx(TxType txType, bytes memory txData, bytes32[] storage transactions) internal {
        ...
        emit TxCreated(transactions.length, txType, txHash);
    ...
    function adjustIssuerAuthorizedShares(
        ...
        bytes16 id = generateDeterministicUniqueID(issuerId);
        ...
        returns (StockAcceptance memory acceptance)
    {
        ...
        return StockAcceptance(id, securityId, comments);
    }
}
